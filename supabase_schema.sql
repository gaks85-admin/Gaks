...
-- =========================================================================
-- EXPLAINABILITY & ANALYTICS ENGINE SCHEMA (watcher_evaluations)
-- =========================================================================
...
-- Enable Row Level Security (RLS) for watcher_evaluations
ALTER TABLE public.watcher_evaluations ENABLE ROW LEVEL SECURITY;

-- Create Policies for watcher_evaluations
CREATE POLICY "Users can read own evaluations"
  ON public.watcher_evaluations
  FOR SELECT
  USING (auth.uid() = user_id);
...
-- Indexes for performance optimization on analytics queries
...
COMMENT ON TABLE public.watcher_evaluations IS 'Stores complete market evaluation logs and analytics trace metrics generated by the Gaks AI Market Watcher pipeline.';
COMMENT ON COLUMN public.watcher_evaluations.watcher_id IS 'References the specific Gaks AI Market Watcher service configuration that triggered the scan.';
COMMENT ON COLUMN public.watcher_evaluations.decision_score IS 'Normalised compliance score calculated by the Weighted Decision Engine.';
